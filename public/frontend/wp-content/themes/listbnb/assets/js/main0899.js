...
(function ($) {
	...
	//search form
	$(".tp-category-box").on("click" ,function (e) {
		e.stopPropagation(); // Prevent the click event from reaching the document level
		$(".tp-category-form-toogle").toggle();
	 });

	 // Category Filter
	 $("#tp-category-input").on("input", function () {
		var userInput = $(this).val().toLowerCase();
		$(".tp-category-form-toogle li").each(function (index) {
			// Skip hiding the first li
			if (index === 0) {
				$(this).show();
				return;
			 }
		   ...
		   if (categoryName.includes(userInput)) {
			  $(this).show();
		   } else {
			  $(this).hide();
		   }
	 ...
	 $(".tp-category-form-toogle li").on("click", function () {
		var selectedCategory = $(this).find("cite").text();
		var slug = $(this).data("slug");

		if(selectedCategory){
		   $(".tp-category-box h2").text(selectedCategory);
		   $(".tp-category-selected").val(slug);
		   $(".tp-category-form-toogle").hide();
		}

	 });

	$(".tp-location-box").on("click" ,function (e) {
		e.stopPropagation(); // Prevent the click event from reaching the document level
		$(".tp-location-form-toogle").toggle();
	 });

	 // Location Filter
	 $("#tp-location-input").on("input", function () {
		var userInput = $(this).val().toLowerCase();
		$(".tp-location-form-toogle li").each(function (index) {
			// Skip hiding the first li
			if (index === 0) {
				$(this).show();
				return;
		   ...
		   if (locationName.includes(userInput)) {
			  $(this).show();
		   } else {
			  $(this).hide();
		   }
	 ...
	 $(".tp-location-form-toogle li").on("click", function () {
		var selectedLocation = $(this).find("cite").text();
		var slug = $(this).data("slug");

		if(selectedLocation){
		   $(".tp-location-box h2").text(selectedLocation);
		   $(".tp-location-selected").val(slug);
		   $(".tp-location-form-toogle").hide();
		}
  
	 });

	 $('.tp-banner-location.tp-header__category-submenu span').on('click', function() {
        var selectedLocation = $(this).text();
        if(selectedLocation){
			$(".tp-selected-location").text(selectedLocation);
			$(".tp-rtcl-location").val(selectedLocation);
		 }
    });

	 $(document).on("click", function (e) {
		var target = $(e.target);
		if (!target.closest(".tp-location-form-toogle").length) {
		   $(".tp-location-form-toogle").hide();
		}
  
		if ( !target.closest(".tp-category-form-toogle").length) {

		   $(".tp-category-form-toogle").hide();
		}
	 });

	$('.tp-hero-tab .nav-item label').on('click', function() {
		$('.tp-hero-tab .nav-item label').removeClass('active');
		$(this).addClass('active');
	});
	...
	$(".click-to-reveal").on("click", function () {
        ...
        let link = $(this).find(".hidden-contact");

        if (type === "phone") {
            link.text(contact);
            link.attr("href", "tel:" + contact);
        } else if (type === "email") {
            link.text(contact);
            link.attr("href", "mailto:" + contact);
        }
    });
